...
const LEGAL_TEXT_BLOCK_1 = `Dit rapport dient uitsluitend ter bepaling van de Waarde in het Economisch Verkeer van het visueel getaxeerde motorvoertuig. De geldigheidsduur van deze taxatie is afhankelijk van uw polisvoorwaarden. Het rapport is beslist géén technische keuring en kan daarom nooit als zodanig worden geïnterpreteerd. Het rapport is niet overdraagbaar en is toepasbaar bij polisvoorwaarden volgens artikel 7:960 B.W. De door ons verrichte taxatie houdt geen enkele garantie in tot het realiseren van de vastgestelde waarde bij inruil of verkoop. Onder waarde in het economisch verkeer wordt verstaan het bedrag dat het voertuig bij vrije verkoop zou opbrengen. Dat op deze taxatie van toepassing zijn de algemene voorwaarden voor Register Makelaars en Register Taxateurs in roerende zaken, leden van de Federatie van Taxateurs, Makelaars en Veilinghouders in roerende zaken, welke voorwaarden zijn gedeponeerd bij de Kamer van Koophandel en Fabrieken voor Amsterdam op 30-06-2005 onder nummer 40530226.`;

const VALUE_INTRO = `Uitgaande van de marktsituatie op het moment van de taxatie stelden wij de Waarde in het Economisch Verkeer van het hier omschreven voertuig, inclusief accessoires, op`;
...
const PDFWevValuationContent = ({ report, pageNumber, totalPages }: PDFWevValuationContentProps) => {
  ...
  const eindwaarde = report.wev_eindwaarde || report.wev_definitief || report.appraised_value;
  const hasValue = eindwaarde && eindwaarde > 0;
  const formattedValue = hasValue ? formatCurrency(eindwaarde) : '€ ___________';
  ...
  return (
    <Page size="A4" wrap={false} style={{ padding: '24 28', fontFamily: 'Helvetica', position: 'relative' }}>
      ...
      <Text style={{ fontSize: 10, color: '#000000', lineHeight: 1.6, marginBottom: 20 }}>
        Aldus, naar beste weten en kunnen opgemaakt te Druten, {formatDateLong(report.wev_finalized_at || null)}.
      </Text>
