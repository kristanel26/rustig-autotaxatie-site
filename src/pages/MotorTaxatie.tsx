...
const MotorTaxatie = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <SiteHeader />
      <LandingHero subtitle="VERZEKERINGSTAXATIE MOTOR" title="Zekerheid over de waarde van je motor" description="Je motor is meer dan alleen een voertuig. Met een verzekeringstaxatie leg je vast wat het op dit moment werkelijk waard is. Die waarde vormt de basis voor de verzekering, zodat je bij schade of diefstal niet afhankelijk bent van een dagwaarde." ctaText="Verzekeringstaxatie aanvragen" onCtaClick={scrollToForm} heroImage={heroMotor} compact />
      <UspBar />
      ...
      <section className="py-10 md:py-14 px-6 md:px-8 bg-background" ref={formRef}>
        ...
        <div className="grid md:grid-cols-[1fr_380px] gap-8"><div><IntakeForm serviceType="motorverzekeringstaxatie" formTitle="Verzekeringstaxatie aanvragen" formSubtext="Vul onderstaand formulier zo volledig mogelijk in." submitButtonText="Verzekeringstaxatie aanvragen" styledKenteken /></div><ContactSidebar /></div></div> formTitle="Verzekeringstaxatie aanvragen" formSubtext="Vul onderstaand formulier zo volledig mogelijk in." submitButtonText="Verzekeringstaxatie aanvragen" styledKenteken /></div><ContactSidebar /></div></div></div><ContactSidebar /></div></div>
      </section>
